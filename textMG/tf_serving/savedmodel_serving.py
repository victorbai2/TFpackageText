...
def build_SavedModel(conv_net, export_path_serving, savedmodel_version, X, best_sess):
    """
    ...
    model = conv_net(X, args.num_classes, args.dropout,
    ...
    prediction_signature = (
        tf.saved_model.signature_def_utils.build_signature_def(
            inputs=tensor_info_input, outputs=tensor_info_output, method_name=method_name
        ))
    key_my_signature = tf.saved_model.signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY
    builder.add_meta_graph_and_variables(
        best_sess, [tf.saved_model.tag_constants.SERVING],
